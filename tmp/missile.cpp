...
public:
  ...
  vec_cart thru_rotation(void){
    ...
    theta_x=z_ax.inter_degree(vec_cart(0, dir.y, dir.z))*PI/180;
    theta_y=z_ax.inter_degree(vec_cart(dir.x, 0, dir.z))*PI/180;
    vec_cart t_dir_car=static_cast<vec_cart>(t_dir);
    t_dir_car=t_dir_car*(-1);
    vec_cart tmp=vec_cart(vec_cart(1, 0, 0)*t_dir_car, vec_cart(0, std::cos(theta_x), -std::sin(theta_x))*t_dir_car, vec_cart(0, std::sin(theta_x), std::cos(theta_x))*t_dir_car);
    return vec_cart(vec_cart(std::cos(theta_y), 0, std::sin(theta_y))*tmp, vec_cart(0, 1, 0)*tmp, vec_cart(-std::sin(theta_y), 0, std::cos(theta_y))*tmp);
  }
